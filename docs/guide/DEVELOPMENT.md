...
chuan-os（川流）是一个**本地运行的多角色 AI 班底**。单入口「幕僚长」按意图路由到 14 个虚拟员工（秘书/律师/编程/研究/投资/新媒体/社交/陪伴/管家/保镖/学习/自由职业），每个角色可绑定不同大脑模型和工具集，重活可调用外来 agent（pi/OpenCode/Claude Code）。
...
├─────────────────────────────────────────────────────┤
│ L3 角色层  14+ 角色（SOUL.md 驱动，独立 brain/工具）  │
├─────────────────────────────────────────────────────┤
...
| ③ Agent Spawner | 动态出生/销毁角色、sub_agent 按需创建 | ✅ 已拆分为独立组件 | `gateway/agent_spawner.py` |
...
| ⑥ Heartbeat | 健康检查、角色状态监控 | ✅ 已拆分为独立组件 | `gateway/heartbeat.py` |
...
Gateway.Session Manager 获取/创建会话
    ↓
Gateway.Agent Spawner 确保 lawyer 角色已出生
    ↓ L3 角色层
lawyer 角色（brain=cloud_general, tools=[filesystem, contract_review]）
    ↓ L2 工具层
...
### 3.1 角色 vs agent vs sub_agent

| 概念 | 定义 | 调度者 | 例子 |
|---|---|---|---|
| **角色（persona）** | 班底成员，有独立人设/职责/工具/大脑 | 幕僚长（Gateway） | 律师、编程、研究 |
| **岗位（role）** | 角色 = 项目经理（ADR-014），只做规划/调度/汇总，不直接执行 | 角色持有 | PersonaRole |
| **agent** | 岗位的执行者（外包工程师），从 agent_pool 取 | 岗位调度 | 内置 ReAct / prime_agent / pi |
| **sub_agent** | 角色可调用的外部 AI agent，对角色来说是工具 | 角色调用 | pi、prime_agent、OpenCode |

**关系**：幕僚长 → 角色（岗位=项目经理）→ agent（干活实例，按复杂度 1:1→1:N）→ sub_agent（重活外包）
...
│   └── voices.yaml            # 角色独立 TTS 音色（14 角色）
│
├── personas/                  # 角色层（SOUL.md 目录驱动，ADR-013，14 个）
│   └── <name>/                # 每个角色一个目录
│       ├── SOUL.md            # 人设/职责/说话风格（agent 可自写）
│       ├── config.yaml        # brain/工具权限/sub_agents
│       └── MEMORY.md          # 角色私有记忆（运行时生成）
│
...
- 每个核心模块对应 tests/test_<module>.py
...
- **N11–N16 能干活→能记住→能说话**：启动修复+基础工具 / 会话持久化+sub_agent / 三层记忆 / SOUL.md 角色驱动 / 语音交互闭环 / 角色独立音色+音效
...
A: LangGraph 提供成熟的 Supervisor 多 agent 编排、checkpointer 持久化、post_model_hook 等能力，自研需要重写 ReAct 循环、工具调用、会话管理等，工作量大且容易出 bug。采框架符合薄层原则（ADR-005/007）。

### Q: 角色和 agent 什么关系？
A: 角色 = 岗位 = 项目经理（ADR-014），只做规划/调度/汇总；agent 是岗位的"执行者"（外包工程师），从 agent_pool 取，按任务复杂度选择（简单→内置 ReAct，中等→prime，重型→pi/OpenCode/Claude Code）。岗位化 1:N 迁移：N37（ADR-032）岗位可持有 N 个 agent 实例 + 多会话并行不串扰；N38（ADR-033）1:N 默认启用（并行 auto 子任务各配独立 worker）；N39（ADR-034）按实例配置工具/模型/记忆（`RoleAgentConfig`）；N40（ADR-035）config.yaml `role_instances` 声明式按复杂度选实例（simple/medium/heavy → 实例，角色可覆盖，改配置即生效）。

### Q: 最终是语音还是 TUI？
A: 语音是最终日常交互方式（ADR-011），TUI 仅用于开发调试和复杂操作。CLI 是最基础的开发入口。

### Q: 为什么不直接用 OpenClaw/Jarvis？
A: chuan-os 的核心价值是"多角色班底调度"——幕僚长路由到不同职责的角色，每个角色可调用不同的外来 agent。OpenClaw 偏单 agent，Jarvis 偏自改进单 agent。chuan-os 做编排层，复用它们的能力。

### Q: 启动报错怎么修？
A: 旧版 `runtime_supervisor.py` 曾调用未实现的 `_create_call_agent_tool()`（sub_agent 功能残留），该调用已随 Gateway 重构删除——`wake_up()` 现在委托 `agent_spawner.spawn()` 出生岗位，不再存在此方法。当前版本可直接 `python -m chuan.main` 启动，无启动报错（见 ROADMAP「已知遗留」已解决记录）。
