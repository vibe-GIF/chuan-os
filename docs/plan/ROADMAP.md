...
| P3 | ✅ GUI 元素记忆库（越用越不用调教：定位成功沉淀「软件+控件→坐标」，下次先查记忆再兜底） → **N58/ADR-055**（`handlers/gui_memory.py` + `data/gui/elements.db` SQLite upsert/hits；gui_locate 命中自动存、未命中查记忆提示；gui_click 记忆坐标兜底；gui_mem_list/forget skill；单测 12+4，真实 DB 冒烟通过，conftest 双模块路径隔离；全量回归 798 passed 无回退） | 自研（对齐 blackboard 落盘） |
