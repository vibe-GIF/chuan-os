...
| P3 | GUI 自动化（借鉴影刀 RPA 能力，补「无 API 软件操作」这条腿） → **N57/ADR-054**（阶段1 截图+元素定位 mss→pywinauto 主/vision_analyze 视觉兜底；阶段2 pywinauto 元素操作主力+pyautogui 坐标兜底，**默认后台静默模式（UIA 不抢焦点）**；阶段3 gui_operate 闭环**双模式并存+动态切换**（后台静默/接管屏幕，自动判定+手动指定）+**静默可见性**（实时状态/留痕/关键动作确认/可中止）+安全闸 guard/超时/截图留痕/冲突检测；阶段4 测试+文档。可选增强 UI-TARS（视觉 GUI agent，MCP 接入），备选 RPA Framework。已列计划+选型+闭环设计，未开工） | 影刀 |
