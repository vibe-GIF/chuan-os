...
## 八、业界调研：TencentDB Agent Memory（2026-08-25）

> 腾讯云数据库团队开源的分层记忆引擎（MIT，2026-05 开源，90 天 2 万+ star，多期 Trending 第一）。
> 与 chuan-os 记忆体系同命题，是验证选型方向 + 提取借鉴点的业界参照。

### 8.1 五维对照

| 维度 | chuan-os（现有） | Agent Memory（腾讯） | 判定 |
|---|---|---|---|
| 记忆分层 | session 巩固 → notes/ + 知识原子「确认/丢弃」（GEPA） | L0 原始 → L1 原子 → L2 场景 → L3 画像 四层蒸馏管线 | **已有**（同思路，实现不同）|
| 知识资产 | wiki 知识库 + skill 沉淀（N30）+ 团队黑板 | Chat / LLM-Wiki / Code Graph / Skill 四类资产目录 | **已有**（chuan 缺 Code Graph）|
| 检索召回 | FTS5 词法 + sqlite-vec 语义双路（N55 闸门） | MemoryProxy 按 team/agent/task 装配 + token 预算控制 | **已有**（chuan 缺按任务装配 + 预算）|
| 长任务上下文 | 黑板落盘 + 团队状态（磁盘真相） | **漏斗模式**：上下文卸载 + Mermaid 任务画布 | **可借**（详见 8.2）|
| 治理 / 审计 | GEPA 自改进 + MEMORY.md | ACL / 版本 / 审计 / 资产目录 | **可借**（企业级阶段 4）|

### 8.2 漏斗模式（上下文卸载）——最值得借鉴的技术

长任务里每次工具返回的完整结果不进上下文，而是按「漏斗」四层递进存放，越往上越轻：

| 层级 | 内容 | 位置 | 说明 |
|---|---|---|---|
| L0 | 完整工具返回原文 | `refs/*.md` | 证据层，最重，按需下钻 |
| L1 | 工具调用级摘要 | `offload.jsonl` | 一行摘要 + 索引路径 |
| L2 | 任务画布节点 | `*.mmd`（Mermaid）| 可导航的任务结构（并行分支/依赖/当前阶段）|
| L3 | 任务级索引（目标+状态） | 上下文 | 最轻，日常驱动推进 |

**原则**：底层保留证据，高层保留结构；Agent 日常只接触 L2-L3 轻量信息，需要时沿 node_id 逐层回溯——**任何一层都可 100% 找回，不是不可逆压缩**。评测：Token 降 31%~61%，成功率反而上升（噪声减少 → 注意力集中）。

**chuan 映射**：`agent_harness` 已有任务 DAG + 终态裁剪（= 画布结构基础），`data/teams/<session>/blackboard/` 已落盘原始产物（= L0 证据层）。**缺的是中间两层**：工具结果卸载到 `refs/` + 由 DAG 生成可导航任务摘要。这是低成本、高收益的增量。

### 8.3 可借鉴 vs 不必抄

**可借鉴**：
- 漏斗模式（上下文卸载 + 任务画布）——解决长任务 token 线性涨、状态不可导航；
- 资产治理（ACL / 版本 / 审计 / 资产目录）——正是企业级阶段 4 的「可治理、可迁移」诉求。

**不必抄**：
- 面向多框架的通用适配层（OpenClaw/Hermes/Claude Code 的 Proxy）——chuan 是自建编排层，不需要通用化；
- 它的评测数字（PersonaMem 48%→76%）是它自己的场景，chuan 的 FTS5+sqlite-vec 在现库规模延迟 <1ms 已够用。

**判断**：该项目印证了 chuan 的选型方向——零外部依赖 SQLite、Markdown/Mermaid 人类可读落盘、按角色装配记忆，与 chuan「磁盘真相 + 人类可读」哲学一致；N55 闸门（库大了再上向量）与它的 token 预算控制同源。**参考它，但不必引入**（对齐 ADR-007 编排层定位）。
